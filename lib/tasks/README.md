...
| File | Role |
|------|------|
| `actions.ts` | `"use server"` — CRUD, status updates (including approval link), subtree delete, tree listing by business, list by agent, `getTaskById`, promote backlog→todo (`promoteTaskToTodo`), dependency + GitHub PR link updates, installation listing |
| `promotion-auth.ts` | `assertMayPromoteToTodo` — human vs agent RBAC for backlog→todo promotion |
